...
const URL_SOFTWARE = `https://raw.githubusercontent.com/eduardoehr89-source/portafolio/main/05_informacion_proyectos/Software_portafolio.csv?t=${timestamp}`;
const URL_NORMS = `https://raw.githubusercontent.com/eduardoehr89-source/portafolio/main/05_informacion_proyectos/Normativas%20y%20Est%C3%A1ndares_portafolio.csv?t=${timestamp}`;
const URL_EDUCATION = `https://raw.githubusercontent.com/eduardoehr89-source/portafolio/main/05_informacion_proyectos/Master%20MBIA_portafolio.csv?t=${timestamp}`;
const URL_ROI = `https://raw.githubusercontent.com/eduardoehr89-source/portafolio/main/05_informacion_proyectos/ROI_Scripts_portafolio.csv?t=${timestamp}`;
...
const getVal = (item, ...keys) => {
    if (!item) return '';
    // 1. Probar llaves directas
    for (const key of keys) {
        if (item[key] !== undefined && item[key] !== null) return item[key];
    }
    // 2. Probar normalización de todas las llaves del objeto (insensible a tildes/case)
    const normalizedKeys = Object.keys(item).reduce((acc, k) => {
        acc[normalizeStr(k)] = k;
        return acc;
    }, {});

    for (const key of keys) {
        const nKey = normalizeStr(key);
        if (normalizedKeys[nKey]) return item[normalizedKeys[nKey]];
    }
    return '';
};
...
window.openSummary = () => {
    ...
    if (window.summaryData && window.summaryData.length > 0) {
        renderSummaryModal(window.summaryData);
    }
};
...
// ==========================================================================
// 3. MOTORES DE RENDERIZADO (GLOBAL)
// ==========================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Promise.allSettled([
        ...
        renderROI(roiData);
        renderCollab(collab);
        renderGlobalMethodology(projectsData);
        ...
        setInterval(() => {
            ...
            renderGlobalMethodology(projectsData, true);
...
function injectSectionHeader(containerId, keyword, defaultTitle, defaultIcon, parentLevel = 1) {
    ...
    if (headerEl) {
        ...
        const navKeywords = ['fase', 'tipologia', 'disciplinas', 'entorno', 'pais', 'ano', 'liderazgo', 'metodologia'];
...
function renderGlobalAnalytics(data, animateOnly = false) {
    ...
    container.innerHTML = chartData.map((d, i) => {
        ...
        return `
        <div class="flex items-center gap-2 db-text-body mb-1 font-mono group animate-item" style="animation-delay: ${i * 0.1}s">
            <div class="flex items-start gap-1 w-24 xl:w-28 shrink-0 pl-1">
...
function renderGlobalPhases(allProjects, animateOnly = false) {
    ...
    stats.forEach((s, idx) => {
        ...
        html += `
        <div class="flex items-center gap-2 db-text-body mb-1 font-mono group animate-item" style="animation-delay: ${delay}s">
            <div class="flex items-start gap-1 w-24 xl:w-28 shrink-0 pl-1">
...
function renderGlobalMethodology(allProjects, animateOnly = false) {
    const parentContainer = document.getElementById('global-environment-list')?.parentElement;
    if (!parentContainer) return;

    // Diseño estable: se reparte el espacio de forma natural sin forzar márgenes que rompan el DOM
    parentContainer.className = "flex flex-row h-full w-full items-start justify-start gap-12";

    let methodologyContainer = document.getElementById('global-methodology-list');
    if (!methodologyContainer) {
        methodologyContainer = document.createElement('div');
        methodologyContainer.id = 'global-methodology-list';
        methodologyContainer.className = "flex flex-col h-full min-w-[180px] overflow-visible";
        parentContainer.appendChild(methodologyContainer);
    }

    let entornoContainer = document.getElementById('global-environment-list');
    if (!entornoContainer) {
        entornoContainer = document.createElement('div');
        entornoContainer.id = 'global-environment-list';
        entornoContainer.className = "flex flex-col h-full min-w-[160px] overflow-visible";
        parentContainer.insertBefore(entornoContainer, methodologyContainer);
    } else {
        entornoContainer.className = "flex flex-col h-full min-w-[180px] overflow-visible";
    }


    const counts = { 'Open BIM': 0, 'Closed BIM': 0 };
    let total = 0;

    allProjects.forEach(p => {
        const rawMet = getVal(p, 'metodología proyecto', 'metodologia');
        const met = normalizeStr(rawMet);
        if (met.includes('open')) {
            counts['Open BIM']++;
            total++;
        } else if (met.includes('closed')) {
            counts['Closed BIM']++;
            total++;
        }
    });
    ...
    if (total === 0) {
        methodologyContainer.innerHTML = '<div class="text-gray-500 text-[10px] text-center italic p-4">Datos no disponibles en CSV</div>';
        return;
    }

    const totalMet = (counts['Open BIM'] || 0) + (counts['Closed BIM'] || 0) || 1;
    const pctOpen = Math.round((counts['Open BIM'] / totalMet) * 100) || 0;
    const pctClosed = Math.round((counts['Closed BIM'] / totalMet) * 100) || 0;
    
    // Título y barras con visibilidad estática absoluta para evitar parpadeos
    methodologyContainer.innerHTML = `
    <div class="flex flex-col h-full w-full overflow-visible">
        <div class="flex items-center justify-between group cursor-pointer hover:bg-white/5 px-2 -mx-2 py-1 rounded-lg transition-all duration-300 w-full overflow-hidden mb-2" 
             onclick="window.switchView('projects', false, false, true); window.setProjectView('groups', 'metodologia')"
             title="Ver Proyectos por METODOLOGÍA">
            <div class="flex items-center gap-2 overflow-hidden">
                <i data-lucide="share-2" class="text-gray-400 group-hover:text-cyan-400 w-4 h-4 shrink-0 transition-colors"></i>
                <span class="db-text-header font-bold truncate group-hover:text-white transition-colors uppercase tracking-wider">METODOLOGÍA</span>
            </div>
            <i data-lucide="chevron-right" class="w-3 h-3 text-gray-600 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all"></i>
        </div>
...
function renderGlobalEnvironment(allProjects, animateOnly = false) {
    ...
    if (parent) {
        // Layout de flujo constante estable para evitar solapamientos
        parent.className = "flex flex-row h-full gap-8 items-start relative overflow-visible";
    }
    ...
    const totalEntorno = obraCount + oficinaCount || 1;

    const pctObra = Math.round((obraCount / totalEntorno) * 100) || 0;
    const pctOficina = Math.round((oficinaCount / totalEntorno) * 100) || 0;

    container.innerHTML = `
    <div class="flex flex-col h-full w-full overflow-visible">
        <div class="flex items-center justify-between group cursor-pointer hover:bg-white/5 px-2 -mx-2 py-1 rounded-lg transition-all duration-300 w-full overflow-hidden mb-2" 
             onclick="window.switchView('projects', false, false, true); window.setProjectView('groups', 'entorno')"
             title="Ver Proyectos por ENTORNO">
            <div class="flex items-center gap-2 overflow-hidden">
                <i data-lucide="map-pin" class="text-gray-400 group-hover:text-cyan-400 w-4 h-4 shrink-0 transition-colors"></i>
                <span class="db-text-header font-bold truncate group-hover:text-white transition-colors uppercase tracking-wider">ENTORNO</span>
            </div>
            <i data-lucide="chevron-right" class="w-3 h-3 text-gray-600 opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all"></i>
        </div>
        <div class="flex flex-row h-[120px] w-full items-end justify-center gap-12 pb-2">
    ...
    const renderSoftwareGroup = (grupoName) => {
        ...
        grouped[grupoName].forEach((s) => {
            ...
            <div class="shrink-0 flex items-center gap-1 bg-cyan-900/10 hover:bg-cyan-500/20 border border-cyan-500/20 hover:border-cyan-400/50 rounded-md px-1.5 py-0.5 transition group cursor-help anim-fade-in"
                ...
                <span class="text-[8.5px] md:text-[9.5px] font-normal uppercase tracking-wider text-white [.light-theme_&]:text-gray-800 transition leading-none py-0.5 whitespace-nowrap">${displayName}</span>
            </div>`;
        });
            ...
            // DiseÃ±o muy compacto (píldora con icono y nombre)
                html += `
                    <div class="shrink-0 flex items-center gap-1 bg-blue-900/10 hover:bg-blue-500/20 border border-blue-500/20 hover:border-blue-400/50 rounded-md px-1.5 py-0.5 transition group cursor-help"
                        ...
                        <span class="text-[8.5px] md:text-[9.5px] font-normal uppercase tracking-wider text-white [.light-theme_&]:text-gray-800 transition leading-none py-0.5 whitespace-nowrap">${title}</span>
                    </div>`;
            });
            html += `</div></div>`;
        });
...
window.showRoiTooltip = function (e) {
    ...
    const isProjectSection = target.closest('#view-projects') !== null;
    const fs = isProjectSection ? '11px' : '10px';

    const isCompact = target.getAttribute('data-tooltip-size') === 'compact';
    const metaHTML = meta ? `<div class="font-mono font-bold uppercase tracking-[0.2em] mb-1 ${colorClass}" style="font-size: ${fs} !important;">${meta}</div>` : '';
    const titleHTML = title ? `<div class="font-sans font-semibold mb-1 tracking-tight ${isCompact ? 'normal-case' : 'uppercase'} tooltip-title leading-tight" style="font-size: ${fs} !important;">${title}</div>` : '';
    const subtitleHTML = subtitle ? `<div class="italic text-gray-500 lowercase font-mono mt-1" style="font-size: ${fs} !important;">${subtitle}</div>` : '';
    const footerHTML = footer ? `<div class="mt-2.5 pt-2 border-t border-gray-700/60 font-mono text-gray-400 leading-normal w-full flex flex-col items-center" style="font-size: ${fs} !important;"><span>${footer}</span></div>` : '';
    ...
    if (isCompact) {
        ...
        // SOLUCIÓN DINÁMICA: Cálculo basado en caracteres para evitar anchos excesivos en textos cortos
        // Mantiene la horizontalidad (Regla 1.4) pero ajustada al contenido real
        const charCount = (text || "").length;
        idealWidth = Math.max(320, Math.min(650, 250 + charCount * 0.7));
    }
    
    // Aplicar dimensiones y estilos
    globalTooltipEl.classList.remove('hidden', 'max-w-3xl', 'max-w-2xl', 'md:max-w-[540px]');
    // Margen reducido para mejor estética (Regla 1.3.1)
    globalTooltipEl.classList.add('flex', 'flex-col', 'items-center', 'justify-start', isCompact ? 'p-2.5' : 'p-6'); 
    
    globalTooltipEl.style.width = `${idealWidth}px`;
    globalTooltipEl.style.minWidth = isCompact ? '180px' : '320px';
    ...
    globalTooltipEl.innerHTML = `
        <div class="flex flex-col items-center justify-start text-center w-full">
            ...
            ${titleHTML}
            ${subtitleHTML}
            <div class="leading-relaxed font-normal text-center w-full font-sans mb-2" style="font-size: ${fs} !important;">
                ${text}
            </div>
            ${footerHTML}
        </div>
...
function renderROI(data, animateOnly = false) {
    const tableContainer = document.getElementById('global-roi-table');
    const gridContainer = document.getElementById('global-roi-grid');

    // Inyectar cabecera una sola vez si no es solo animación
    ...
    data.forEach((i, index) => {
        try {
            ...
            const manual = getVal(i, 't. manual', 'tiempo manual', 'manual');
            const auto = getVal(i, 'auto.', 'tiempo script', 'tiempo optimizado', 'auto');
            ...
            else if (toolNameLow.includes('tuberías') || toolNameLow.includes('tuberias') || toolNameLow.includes('ruteado')) icon = 'workflow';
            ...
            else if (toolNameLow.includes('láminas') || toolNameLow.includes('laminas') || toolNameLow.includes('planos')) icon = 'layout-template';
            else if (toolNameLow.includes('estandarizador') || toolNameLow.includes('bep')) icon = 'clipboard-check';
            else if (toolNameLow.includes('gestor') || toolNameLow.includes('cuantificación') || toolNameLow.includes('familias')) icon = 'calculator';
            else if (toolNameLow.includes('etiquetado')) icon = 'tag';
            else if (toolNameLow.includes('salud') || toolNameLow.includes('monitor')) icon = 'heart-pulse';
            else if (toolNameLow.includes('vistas')) icon = 'eye';
            else if (toolNameLow.includes('gemini')) icon = 'brain-circuit';
            else if (toolNameLow.includes('banana')) icon = 'sparkles';
            else if (toolNameLow.includes('flow')) icon = 'wind';
            else if (toolNameLow.includes('bimlogic')) icon = 'cpu';
            else if (toolNameLow.includes('pre-flight') || toolNameLow.includes('plugin')) icon = 'rocket';
            else icon = resolveIcon(i, 'zap');

            let speedup = getVal(i, 'vel.', 'ahorro', 'speedup', 'velocidad', 'eficiencia', 'vel', 'x');
            if (speedup) speedup = String(speedup).toLowerCase().replace('x', '').trim();
            else speedup = "0";

            const rawDetail = detail || "Detalle de optimización no disponible.";
            const safeDetail = String(rawDetail).replace(/"/g, '&quot;').replace(/'/g, '&#39;').replace(/\r?\n/g, '<br>');
            ...
            // HTML para Fila de Tabla
            tableHtml += `
            <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/30 transition group cursor-help relative hover:z-50" ${tooltipAttrs}>
                <td class="py-2 pl-2 db-text-meta font-mono text-[10px] uppercase text-gray-400 [.light-theme_&]:text-slate-500">${discipline}</td>
                <td class="py-2 flex items-center gap-2">
                     <div class="p-1 px-1.5 rounded-md transition bg-green-500/10 text-green-500 group-hover:bg-green-500/20 group-hover:text-green-400 shrink-0">
                        <i data-lucide="${icon}" class="w-3.5 h-3.5 stroke-[2.5px]"></i> 
                    </div>
                    <span class="db-text-meta font-mono text-[11px] text-white [.light-theme_&]:text-gray-900 font-bold">${tool}</span>
                </td>
                <td class="py-2">
                    <span class="db-text-body leading-tight text-gray-300 [.light-theme_&]:text-slate-700 text-[11px]" style="font-weight: 300 !important;">${task}</span>
                </td>
                <td class="py-2 text-right db-text-meta font-mono text-[10px] text-gray-400 [.light-theme_&]:text-slate-500">${manual}</td>
                <td class="py-2 pr-2 text-right text-green-700 dark:text-green-400 font-mono text-[10px] db-text-meta">${auto}</td>
                <td class="py-2 pr-2 text-right">
                    <span class="text-[10px] font-bold px-1.5 py-0.5 rounded shadow-[0_0_10px_rgba(74,222,128,0.3)] bg-green-400 text-black">
                        <span class="counter-anim" data-target="${speedup}">0</span>x
                    </span>
                </td>
            </tr>`;

            // HTML para Tarjeta de Cuadrícula
            gridHtml += `
            <div class="roi-card group cursor-help anim-fade-in" ${tooltipAttrs}>
                <div class="roi-card-speedup"><span class="counter-anim" data-target="${speedup}">0</span>x</div>
                <div class="roi-card-icon-container">
                    <i data-lucide="${icon}" class="w-5 h-5 stroke-[2.5px]"></i>
                </div>
                <div class="flex flex-col">
                    <span class="roi-card-discipline">${discipline}</span>
                    <h4 class="roi-card-title">${tool}</h4>
                </div>
                <div class="roi-card-footer">
                    <span class="text-[9px] font-mono text-gray-500 uppercase tracking-tighter">${manual} manual vs ${auto} auto</span>
                    <i data-lucide="chevron-right" class="w-3 h-3 text-green-500 opacity-0 group-hover:opacity-100 transition-all"></i>
                </div>
            </div>`;

        } catch (e) { console.error("Error rendering ROI item:", i, e); }
    });
...
window.switchROIView = function(view) {
    const listView = document.getElementById('roi-view-list');
    const gridView = document.getElementById('roi-view-grid');
    const btnList = document.getElementById('btn-roi-list-v2');
    const btnGrid = document.getElementById('btn-roi-grid-v2');
    ...
    if (view === 'grid') {
        ...
        btnGrid.classList.add('text-green-400');
        btnGrid.classList.remove('text-gray-500');
        btnList.classList.remove('text-green-400');
        btnList.classList.add('text-gray-500');
    } else {
        gridView.classList.add('hidden');
        listView.classList.remove('hidden');
        btnList.classList.add('text-green-400');
        btnList.classList.remove('text-gray-500');
        btnGrid.classList.remove('text-green-400');
        btnGrid.classList.add('text-gray-500');
    }

    // Guardar preferencia (opcional)
    window.lastROIView = view;
    if (window.lucide) window.lucide.createIcons();
};
...
function processProjects(data) {
    ...
    projectsData = data.map((d, i) => {
        ...
        return {
            ...
            metodologia: getVal(d, 'metodología proyecto', 'metodologia'),
...
function updateSystemStatus() {
    ...
    const fMetodologia = document.getElementById('filter-metodologia')?.value || 'all';
    ...
    if (fMetodologia !== 'all') activeFilters.push(`MET:${fMetodologia.toUpperCase()}`);
...
window.runFilter = function () {
    ...
    const fEntorno = document.getElementById('filter-entorno')?.value || 'all';
    const fPhase = document.getElementById('filter-phase')?.value || 'all';
    const fMet = document.getElementById('filter-metodologia')?.value || 'all';
    ...
    filteredProjects = projectsData.filter(p => {
        ...
        const mMet = checkMatch(getVal(p, 'metodología proyecto', 'metodologia'), fMet);
        ...
        return matchesText && matchesTop5 && mLoc && mType && mYear && mDisc && mEntorno && mPhase && mMet && mIA && mVideo && matchesLider;
...
function populateAllFilters() {
    const getOptions = (...keys) => {
        ...
        projectsData.forEach(p => {
            ...
            if (val) {
                // Soportar múltiples tags separados por ";" o ","
                const tags = val.split(/[;,]/).map(t => t.trim().toUpperCase()).filter(Boolean);
                raw.push(...tags);
            }
        });
    ...
    setOptions('filter-metodologia', 'METODOLOGÍA: TODAS', getOptions('metodología proyecto', 'metodologia'));
...
window.clearAllProjectFilters = function () {
    console.log("Filtros: Limpiando todos los criterios de búsqueda y filtrado...");
    const filters = ['filter-loc', 'filter-type', 'filter-year', 'filter-disc', 'filter-entorno', 'filter-phase', 'filter-metodologia', 'filter-liderazgo', 'filter-ia', 'filter-video'];
    filters.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = 'all';
    });
...
window.setProjectView = function (mode, groupBy = null, isPopstate = false) {
    ...
    if (groupBy) {
        // v50.51 - Si cambia la agrupación en modo Grupos, reseteamos filtros para evitar persistencia errónea
        if (mode === 'groups' && window.currentProjectsGrouping !== groupBy) {
            window.clearAllProjectFilters();
        }
...
window.groupToGallery = function (groupValue, type, isPopstate = false) {
    ...
    const filterIds = {
        ...
        'metodologia': 'filter-metodologia', // Added methodology filter
...
window.goBackToGroups = function () {
    ...
    const filters = ['filter-loc', 'filter-type', 'filter-year', 'filter-disc', 'filter-entorno', 'filter-phase', 'filter-metodologia', 'filter-ia', 'filter-video', 'filter-liderazgo'];
    filters.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.value = 'all';
    });
...
function renderProjects(list) {
    ...
    if (currentViewMode === 'groups') {
        ...
        groupsContainer.innerHTML = `
            <div class="pt-0 px-6 pb-6 w-full animate-fade-in">
                <div class="flex items-center justify-end mb-4">
                    <div class="flex gap-2">
                        ...
                        <button onclick="window.setProjectView('groups', 'metodologia')" 
                                class="px-3 py-1.5 rounded-lg border ${groupingKey === 'metodologia' ? 'border-cyan-500/50 bg-cyan-500/10 text-cyan-400' : 'border-gray-700 text-gray-500'} hover:border-cyan-500 transition-all text-[9px] font-mono uppercase tracking-widest">METODOLOGÍA</button>
...
window.showRetoTooltip = function (btn, event, projectId) {
    ...
    // Content
    globalTooltipEl.innerHTML = `<div class="leading-tight font-normal font-mono px-1 flex-1 text-center" style="font-size: 11px !important;">${retoFormatted}</div>`;
...
window.showUbicacionTooltip = function (btn, event, projectId) {
    ...
    // Content
    globalTooltipEl.innerHTML = `
        <div class="font-sans font-bold mb-2 border-b pb-2 uppercase tracking-wider border-gray-700 [.light-theme_&]:border-gray-200 shrink-0 tooltip-title text-center w-full" style="font-size: 11px !important;">
            UBICACIÓN
        </div>
        <div class="leading-relaxed font-normal font-mono pr-1 flex-1 text-center w-full" style="font-size: 11px !important;">
           ${project.ubicacion || ''}
        </div>
    `;
