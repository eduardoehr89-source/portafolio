...
function renderGlobalAnalytics(data, animateOnly = false) {
    ...
    let legendHtml = chartData.map((d, i) => `
        <div class="flex items-center justify-between db-text-body mb-1 font-mono group animate-item ml-2" style="animation-delay: ${i * 0.1}s">
            ...
            <span class="db-text-body text-gray-400 w-10 text-right shrink-0"><span class="counter-anim" data-target="${Math.round((d.v / totalTop4) * 100)}">0</span>%</span>
        </div>
    `).join('');
...
function renderGlobalDisciplines(allProjects, refDisciplines, animateOnly = false) {
    ...
    container.innerHTML = top5Stats.map((stat, i) => {
        ...
        <div class="flex items-center justify-between db-text-body mb-0.5 font-mono group">
            ...
            <span class="db-text-body text-gray-400 w-8 text-right"><span class="counter-anim" data-target="${percentage}">0</span>%</span>
        </div>`;
    }).join('');
...
function renderGlobalPhases(allProjects, animateOnly = false) {
    ...
    stats.forEach((s, idx) => {
        ...
        <div class="flex items-center justify-between db-text-body mb-0.5 font-mono group animate-item" style="animation-delay: ${delay}s">
            ...
            <span class="db-text-body w-8 text-right text-gray-400"><span class="counter-anim" data-target="${pct}">0</span>%</span>
        </div>`;
        delay += 0.05;
    });
...
function renderEducation(data) {
    ...
    if (headerContainer) {
        ...
        headerContainer.insertAdjacentHTML('beforeend', `
            <div class="flex items-center gap-2 ml-3 opacity-90 transition hover:opacity-100">
                ...
                <a href="https://butic.es/" target="_blank" class="text-green-500 [.light-theme_&]:text-green-700 hover:text-cyan-400 hover:underline transition text-xs font-mono ml-1">Butic The New School <i data-lucide="external-link" class="inline w-2.5 h-2.5 mb-0.5"></i></a>
                <span class="text-gray-400 [.light-theme_&]:text-slate-500 text-[10px] font-mono border border-gray-700 [.light-theme_&]:border-slate-500 px-1.5 py-0.5 rounded ml-1">ESPAÑA</span>
                <span class="text-gray-400 [.light-theme_&]:text-slate-500 text-[10px] font-mono border border-gray-700 [.light-theme_&]:border-slate-500 px-1.5 py-0.5 rounded ml-1">ONLINE</span>
                <span class="text-green-400 [.light-theme_&]:text-green-800 text-[10px] font-mono border border-green-700/50 [.light-theme_&]:border-green-600 bg-green-900/10 px-1.5 py-0.5 rounded ml-1">EN CURSO</span>
                <a href="https://www.credential.net/c455ea9f-59d7-44d6-9d15-c81eb34d5cfb" target="_blank" class="text-blue-400 [.light-theme_&]:text-blue-800 hover:text-white hover:bg-blue-600 transition text-[10px] font-mono border border-blue-700/50 [.light-theme_&]:border-blue-600 bg-blue-900/10 px-1.5 py-0.5 rounded ml-1 cursor-pointer flex items-center gap-1">INSIGNIA <i data-lucide="award" class="w-2.5 h-2.5"></i></a>
            </div>`);
    }
    ...
    container.innerHTML = data.map(e => {
        ...
        const tooltipHTML = `${safeAccionTecnica}<br><br><span class='text-[10px] !text-white opacity-80 font-mono tracking-wider'>HERRAMIENTAS CLAVE:</span><br><span class='text-xs font-mono text-cyan-400'>${safeHerramientas}</span>`;
        ...
        return `
            <div class="relative cursor-help p-2 bg-transparent hover:border-green-500/50 transition flex flex-col items-center justify-center text-center gap-1.5 h-full w-full group"
                 ...
                 onmouseleave="window.hideRoiTooltip()"
                 onmousemove="window.moveRoiTooltip(event)">
            <div class="shrink-0 p-1.5 rounded bg-green-500/10 [.light-theme_&]:bg-green-700/20 text-green-500 [.light-theme_&]:text-green-700 group-hover:bg-green-500/20 group-hover:text-white transition flex items-center justify-center">
                <i data-lucide="${icon}" class="w-5 h-5 [.light-theme_&]:stroke-[2.5px]"></i>
            </div>
            <div class="min-w-0 w-full">
                <h4 class="text-[10px] sm:text-xs font-light uppercase tracking-wider text-gray-200 [.light-theme_&]:text-black transition mb-0 text-center leading-tight">${title}</h4>
            </div>
        </div > `;
    }).join('');
...
window.showRoiTooltip = function (e) {
    if (!window.globalHoverTooltip) {
        ...
        window.globalHoverTooltip.className = `fixed w-max max-w-xs md:max-w-sm px-4 py-3 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.8)] z-[99999] pointer-events-none transition-opacity duration-200 opacity-0
        text-gray-200
        border border-gray-600/30`;
        window.globalHoverTooltip.style.cssText = "backdrop-filter: blur(16px) !important; -webkit-backdrop-filter: blur(16px) !important; background-color: rgba(15, 20, 26, 0.5) !important; color: #ffffff !important;";
    ...
    const titleHTML = title ? `
        <div class="font-sans font-semibold mb-2 border-b pb-1 uppercase tracking-wider text-xs text-green-400 border-gray-700/50">${title}</div>` : '';

    window.globalHoverTooltip.innerHTML = `
        ${titleHTML}
        <div class="leading-relaxed font-normal text-xs text-justify w-full !text-white text-shadow-sm">
            ${text}
        </div>
    `;
        ...
        try {
            ...
            <tr class="border-b border-gray-800/50 last:border-0 hover:bg-white/5 transition group cursor-pointer relative hover:z-50" onclick="openROIModal(${index})">
                ...
                <td class="py-2 flex items-center gap-2">
                    ...
                    <span class="db-text-meta font-mono text-xs text-cyan-500 [.light-theme_&]:text-cyan-800 font-medium">${tool}</span>
                </td>
...
function renderCollab(data) {
    ...
    const genItem = (item, colorKey, isOffice = false) => {
        ...
        const lightThemeStroke = isOffice ? '[.light-theme_&]:!stroke-green-600' : '[.light-theme_&]:!stroke-blue-500';
        const lightThemeText = isOffice ? '[.light-theme_&]:!text-green-700' : '[.light-theme_&]:!text-blue-700';
        const lightThemeBg = isOffice ? '[.light-theme_&]:!bg-green-500/10' : '[.light-theme_&]:!bg-blue-500/10';
        const lightThemeBorder = isOffice ? '[.light-theme_&]:!border-green-500/30' : '[.light-theme_&]:!border-blue-500/30';
        ...
        <div id="${uniqueId}" class="collab-item group relative p-1.5 rounded-md border border-transparent transition-all cursor-pointer bg-transparent w-full"
            ...
            <div class="flex items-center gap-2 ${flexRowClass}">
                <div class="w-6 h-6 rounded shrink-0 bg-[#0f141a] ${lightThemeBg} border border-gray-800 ${lightThemeBorder} flex items-center justify-center transition shadow-sm ${colorClassBase}">
                ...
                <div class="flex flex-col min-w-0 ${textAlignClass}">
                    <span class="db-text-body font-bold text-white [.light-theme_&]:text-slate-800 text-xs ${hoverColorClassBase} ${lightThemeText} font-mono transition leading-tight">${role}</span>
                </div>
            </div>
        </div>`;
    };
...
window.showCollabTooltip = function (role, inData, outData) {
    ...
    // Quitar estilos de caja (fondo, borde, blur) para que la información flote
    popup.style.cssText = "background-color: transparent !important; border: none !important; backdrop-filter: none !important; padding: 0.5rem;";
    ...
    projectsData = data.map((d, i) => {
        ...
        return {
            ...
            get reto() {
                ...
                formatted = formatted.replace(/(?:^|\s)(Planeacion:|Planeación:)(?=\s)/gi, '<br><strong style="color: #00e5ff !important; font-weight: 700;">$1</strong>');
                formatted = formatted.replace(/(?:^|\s)(Ejecucion:|Ejecución:)(?=\s)/gi, '<br><br><strong style="color: #00e5ff !important; font-weight: 700;">$1</strong>');
                formatted = formatted.replace(/(?:^|\s)(Cierre:)(?=\s)/gi, '<br><br><strong style="color: #00e5ff !important; font-weight: 700;">$1</strong>');
...
function renderProjects(list) {
    ...
    if (listContainer) {
        listContainer.innerHTML = list.map(p => {
            ...
            return `
            <div class="tech-list-row grid grid-cols-[40px_10%_15%_12%_8%_4%_8%_8%_6%_6%_1fr] gap-4 border-b border-gray-800 p-3 text-white hover:bg-white/5 cursor-pointer items-start text-xs font-mono" onclick="if(!event.target.closest('button')) openGallery('${p.id}', event)">
                ...
                <div class="text-cyan-400/70 [.light-theme_&]:text-sky-800 font-bold text-[10px] leading-tight break-words">${p.software}</div>
            </div>`;
        }).join('');
    }
    ...
    if (galleryContainer) {
        galleryContainer.innerHTML = list.map(p => {
            ...
            return `
            <div class="group relative aspect-[4/3] bg-transparent rounded-lg overflow-hidden cursor-pointer border border-gray-800 hover:border-cyan-500 transition" onclick="openGallery('${p.id}', event)">
...
window.openGallery = function (id, event) {
    // Lógica del Popup de Advertencia de IA (Basado en VARIABLE GLOBAL para resetear con F5)
    const aiNoticeSeen = window.aiNoticeSeen;
    const aiPopup = document.getElementById('ai-notice-popup');

    if (!aiNoticeSeen && aiPopup) {
        // Posicionamiento centrado en el viewport (más fiable que relativo al clic)
        ...
        // Inicializar iconos de Lucide (por si acaso no se han renderizado)
        if (window.lucide) {
            window.lucide.createIcons();
        }

        // Marcar como visto EN ESTA SESIÓN VIVA (F5 lo resetea a false)
        ...
        // IMPORTANTE: Detener la apertura del modal en el primer clic
        if (event) {
...
function updateModalContent() {
    ...
    // Extraer nombre del espacio del nombre de archivo
    let spaceName = "";
    if (currentSrc && !currentSrc.includes('undefined')) {
        const filename = currentSrc.split('/').pop();
        spaceName = filename.split('.')[0].replace(/_/g, ' ');
    }
    let spaceNameHtml = (spaceName && currentGalleryView === 'default') ? `<span class="text-[#00e5ff] [.light-theme_&]:text-sky-800 ml-2">| ${spaceName}</span>` : "";
    detailsContainer.innerHTML = `${locationDisplay} <span class="text-cyan-500/50 [.light-theme_&]:text-sky-800/60 mx-1">|</span> ${p.anioStr} <span class="text-cyan-500/50 [.light-theme_&]:text-sky-800/60 mx-1">|</span> ${p.tipologia} <span class="text-cyan-500/50 [.light-theme_&]:text-sky-800/60 mx-1">|</span> ${p.entorno} <span class="text-cyan-500/50 [.light-theme_&]:text-sky-800/60 mx-1">|</span> ${p.fase} ${spaceNameHtml}`;

    // 2. GESTIÓN DE TRANSICIÓN DE MEDIOS (Cross-fade optimizado)
    const isFirstLoad = !imgEl.style.backgroundImage && (!videoEl || !videoEl.src);
    if (!isFirstLoad) {
        imgEl.classList.remove('media-visible');
        if (videoEl) videoEl.classList.remove('media-visible');
        imgEl.classList.add('media-transitioning');
        if (videoEl) videoEl.classList.add('media-transitioning');
    }

    // Delay de 220ms para que el fade-out de 200ms termine antes de cambiar el src
    setTimeout(() => {
        if (!videoEl) {
        ...
        if (isVideo) {
            ...
            if (currentSrc && currentSrc !== 'undefined') {
                ...
                imgEl.innerText = "";
                imgEl.className = "w-full h-full bg-contain bg-center bg-no-repeat";
            } else {
                imgEl.style.backgroundImage = 'none';
                imgEl.innerText = "CONFIDENCIAL";
                imgEl.className = "w-full h-full flex items-center justify-center text-gray-500 font-bold tracking-[1em] text-2xl md:text-4xl bg-[#1a202c]";
            }
        }

        counterEl.innerText = p.imagenes ? `${currentImageIndex + 1} / ${p.imagenes.length}` : "1 / 1";

        // Fade-in lento (0.6s definidos en CSS)
        requestAnimationFrame(() => {
            imgEl.classList.remove('media-transitioning');
            if (videoEl) videoEl.classList.remove('media-transitioning');
            imgEl.classList.add('media-visible');
    ...
    if (descEl && retoEl && textBlock) {
        ...
        let extraHtml = extraInfo.length > 0 ? `<span class="!text-cyan-400 ml-1 opacity-100">| ${extraInfo.join(' | ')}</span>` : '';
        ...
        // Mostrar bloque de texto solo si hay contenido y estamos en la primera imagen (estilo carrusel)
        const shouldShow = hasText && currentImageIndex === 0 && currentGalleryView === 'default';
        textBlock.classList.toggle('hidden', !shouldShow);
